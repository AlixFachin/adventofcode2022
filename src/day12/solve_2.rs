...
fn get_shortest_path(game_data: &GameData, start_row: isize, start_col: isize) -> Option<i32> {
    let mut visited_table = vec![vec![false; game_data.nr_cols]; game_data.nr_rows];
...
pub fn solve() {
    ...
    let mut shortest_hike_length = 200_000;
    for (start_r, start_c) in &game_data.start {
        let algo_result = get_shortest_path(&game_data, *start_r, *start_c);
        if let Some(cur_value) = algo_result {
            if cur_value < shortest_hike_length {
                shortest_hike_length = cur_value;
            }
        }
    }

    println!("Result of question 2 is {}", shortest_hike_length);
}
