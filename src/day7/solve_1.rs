...
impl FileSystem {
    ...
    fn get_all_sizes(&self) -> HashMap<usize, u64> {
        let mut all_sizes_vec = HashMap::new();
        for i in 0..self.dir_list.len() {
            all_sizes_vec.insert(i, self.get_size(i));
        }
...
pub fn solve() {
    ...
    let all_sizes = main_fs.get_all_sizes();
    println!("All sizes = {:?}", all_sizes);

    let mut sum_sizes: u64 = 0;
    for (_subfolder_name, subfolder_size) in all_sizes {
        if subfolder_size < 100000 {
            sum_sizes += subfolder_size;
        }
    }
    println!("The solution of question 1 is {}", sum_sizes);
}
